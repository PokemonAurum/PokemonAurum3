...
BOOL CalcAccuracy(void *bw, struct BattleStruct *sp, int attacker, int defender, int move_no) {
    ...
    if (sp->battlemon[attacker].condition3 & CONDITION3_BLINDED) {
        accuracyModifier = QMul_RoundUp(accuracyModifier, UQ412__0_75); // 75% accuracy
    }
...
u8 LONG_CALL CalcSpeed(void *bw, struct BattleStruct *sp, int client1, int client2, int flag) {
    ...
    if ((ability1 == ABILITY_QUICK_FEET) && HasAnyPersistentOrVolatileStatusCondition(sp->battlemon[client1])) {
        speedModifier1 = QMul_RoundUp(speedModifier1, UQ412__1_5);
    }

    if ((ability2 == ABILITY_QUICK_FEET) && HasAnyPersistentOrVolatileStatusCondition(sp->battlemon[client2])) {
